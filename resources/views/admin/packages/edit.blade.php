...
<body class="bg-gradient-to-br from-blue-50 to-indigo-100 min-h-screen">
    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <!-- Page Header -->
        <div class="mb-8">
            <h1 class="text-4xl font-bold text-gray-900 mb-2">Edit Paket</h1>
            <p class="text-lg text-gray-600">Edit informasi paket "{{ $package->name }}"</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-8">
            <form action="{{ route('admin.packages.update', $package) }}" method="POST" enctype="multipart/form-data" x-data="{ mode: '{{ $package->mode }}' }">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="destination" id="destination" value="{{ old('destination', $package->destination) }}" required
                                   class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="price" id="price" value="{{ old('price', number_format($package->price, 0, ',', '.')) }}" required
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <select name="mode" id="mode" required onchange="toggleCategory()"
                                ...
                                <option value="travel" {{ old('mode', $package->mode) == 'travel' ? 'selected' : '' }}>Travel</option>
                                <option value="umroh" {{ old('mode', $package->mode) == 'umroh' ? 'selected' : '' }}>Umroh</option>
                            </select>
                        </div>

                        <div id="categoryDiv" style="display: {{ old('mode', $package->mode) == 'travel' ? 'block' : 'none' }};">
                            <label for="category" class="block text-sm font-semibold text-gray-700 mb-2">Kategori</label>
                            <select name="category" id="category"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                                <option value="">Pilih Kategori</option>
                                <option value="domestik" {{ old('category', $package->category) == 'domestik' ? 'selected' : '' }}>Domestik</option>
                                <option value="internasional" {{ old('category', $package->category) == 'internasional' ? 'selected' : '' }}>Internasional</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            @if($package->image)
                                <div class="mb-4">
                                    <img src="{{ asset('storage/' . $package->image) }}" alt="{{ $package->name }}" class="w-32 h-24 object-cover rounded-lg border">
                            ...
                            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors">
                                ...
                                <p class="mt-2 text-sm text-gray-500">Format: JPG, PNG, GIF. Maksimal 10MB. Kosongkan jika tidak ingin mengubah gambar.</p>
                ...
                <div class="mt-8">
                    ...
                    <textarea name="description" id="description" rows="6" required
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                              placeholder="Jelaskan detail paket, fasilitas yang disediakan, dan informasi penting lainnya...">{{ old('description', $package->description) }}</textarea>
                </div>
